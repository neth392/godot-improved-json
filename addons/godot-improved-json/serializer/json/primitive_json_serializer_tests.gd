## Debug class used to test which types work with [PrimitiveSerializer].
extends Node
...
func _ready() -> void:
	for type: Variant.Type in types_dict:
		...
		var serializer: Serializer = PrimitiveSerializer.new(type)
		var value: Variant = type_examples[type]
		assert(type == typeof(value), "type example mismatch for type %s" % type_name)
		
		Serialization.add_serializer(serializer)
		
		var json_string: String = Serialization.stringify(value)
		var parsed: Variant = Serialization.parse(json_string)
		
		Serialization.remove_serializer(serializer)
