...
func _serialize(instance: Variant, impl: SerializationImpl) -> Variant:
	...
	var typed_dict: Dictionary = SerializationUtil.create_type_dict(impl, array.get_typed_builtin(), 
...
func _deserialize(serialized: Variant, impl: SerializationImpl) -> Variant:
	...
	if dict.has("t"): # Non-object typed array
		...
		assert(config != null, "no SerializationObjectConfig found with id (%s) when deserializing array (%s)" \
