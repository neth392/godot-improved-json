...
## specific type of [Object]. Must be registered to the project's [SerializationObjectConfigRegistry]
## to be used.
@tool
class_name SerializationObjectConfig extends Resource

## The ID of this [SerializationObjectConfig], stored in the serialized data to detect
...
@export var id: StringName:
	set(value):
		if !Engine.is_editor_hint() && _id_initialized:
			assert(false, "can't change SerializationObjectConfig.id at runtime")
		id = value
...
@export_custom(PROPERTY_HINT_TYPE_STRING, &"Object") var for_class: String:
	set(value):
		...
		if Engine.is_editor_hint() && !value.is_empty():
			var script_path: String = SerializationUtil.get_script_path_from_class_name(value)
...
## The [SerializationInstantiator] used anytime an object of this type is being deserialized
## but the property's assigned value is null and thus an instance needs to be created.
## See that class's docs for more info.
@export var instantiator: SerializationInstantiator
...
## [br]NOTE: If any [SerializableProperty] in [member properties] contains the same
## [member SerializableProperty.property_name] as any property in the extended config, it will
## override the one in the extended config. Can be useful to disable properties or
## handle missing properties differently.
@export var extend_other_config: SerializationObjectConfig:
	set(value):
...
## The [SerializableProperty]s that are to be serialized. Properties with [member SerializableProperty.enabled]
## as false are ignored. The order of this array is important as it determines in which order
## properties are serialized in.
## [br]Format: [member SerializableProperty.key]:[SerializableProperty]
@export var properties: Array[SerializableProperty]:
	set(value):
		properties = value
		_editor_update()
...
## resource path or [member ResourceFileInstance.id] is included (see below options).
...
## Array of [ResourceFileInstance]s that must contain every [Resource] file instance
...
@export var json_res_resource_file_instances: Array[ResourceFileInstance]:
	set(value):
...
## [SerializationObjectConfigRegistry] or not.
var registered: bool:
	get():
		return Serialization.object_config_registry.has_config(self)
	set(value):
...
## [member ResourceFileInstance.id]:[ResourceFileInstance]
...
## Returns a new [Array] of all [SerializableProperty]s of this instance and [member extend_other_config]
## (if it isn't null).
func get_properties_extended() -> Array[SerializableProperty]:
	var extended: Array[SerializableProperty] = []
	var names: Dictionary = {}
	
	# Add these properties
	for property: SerializableProperty in properties:
	...
	if extend_other_config != null:
		for property: SerializableProperty in extend_other_config.get_properties_extended():
...
func get_class_script() -> Script:
	...
	var script_path: String = SerializationUtil.get_script_path_from_class_name(for_class)
	if script_path.is_empty():
		return null
	return load(script_path) as Script


## Returns true if this [SerializationObjectConfig] is for a resource.
...
func _populate_resource_dictionaries(array: Array[ResourceFileInstance]) -> void:
	...
	for instance: ResourceFileInstance in array:
		if instance == null:
			push_warning("SerializationObjectConfig(%s) has a null value in json_res_file_instances" \
			 % id)
			continue
		
		assert(!instance.id.is_empty(), ("json_res_file_instances contains a ResourceFileInstance " + \
		"with an empty ID for SerializationObjectConfig(%s)") % id)
		
		assert(instance.resource != null, ("ResourceFileInstance(%s) does not have a resource " + \
		"set, found in SerializationObjectConfig(%s)") % [instance.id, id])
		
		assert(!instance.path_to_resource.is_empty(), ("ResourceFileInstance(%s) has an " + \
		"empty path, found in SerializationObjectConfig(%s)") % [instance.id, id])
		
		assert(!_file_instances_by_path.has(instance.path_to_resource), ("duplicate " + \
		"resource paths (%s) found in json_res_file_instances of SerializationObjectConfig(%s)") \
		% [instance.path_to_resource, id])
		
		assert(!_file_instance_by_id.has(instance.id), ("duplicate ResourceFileInstance.ids " + \
		"(%s) found in json_res_file_instances of SerializationObjectConfig(%s)") % [instance.id, id])
...
func _editor_update() -> void:
	...
	for instance: ResourceFileInstance in json_res_resource_file_instances:
		if instance != null:
			instance._editor_type_hint = for_class


func _to_string() -> String:
	return "SerializationObjectConfig(id=%s,for_class=%s)" % [id, for_class]
