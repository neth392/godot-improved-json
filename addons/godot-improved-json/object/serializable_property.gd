...
## The key of the property in the serialized data.
## [br]WARNING: Changing this property will break existing saves. 
...
@export var property_name: String:
	set(value):
		property_name = value
		if Engine.is_editor_hint() && key.is_empty() && !property_name.is_empty():
			key = property_name
...
## is passed to [method Serializer._deserialize_into]. If false, a new value is constructed
## from the Serializer via [method Serializer._deserialize]. Only supported for specific
...
var _editor_script: Script:
	...
	get():
		...
		if _editor_class_name.is_empty() || ClassDB.class_exists(_editor_class_name):
			return null
		var script_path: String = SerializationUtil.get_script_path_from_class_name(_editor_class_name)
	...
	if property.name == "property_name":
		...
		if script != null:
			for script_property: Dictionary in script.get_script_property_list():
				...
				and Serialization.is_type_serializable(script_property.type):
		...
		if ClassDB.class_exists(base_type):
			...
			for class_property: Dictionary in ClassDB.class_get_property_list(base_type, false):
				...
				and Serialization.is_type_serializable(class_property.type):
	...
	if property.name == "deserialize_into":
		for class_property: Dictionary in _editor_get_class_properties():
			if property_name == class_property.name:
				if !Serialization.can_deserialize_into_type(class_property.type):
...
func _to_string() -> String:
	return "SerializableProperty(key=%s,property_name=%s)" % [key, property_name]
