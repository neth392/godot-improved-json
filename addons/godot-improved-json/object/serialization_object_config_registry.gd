## A registry of all [SerializationObjectConfig]s. Create an instance of this file
...
## [member _configs] so that the Serialization can recognize them.
@tool
class_name SerializationObjectConfigRegistry extends Resource

## Default configs built into Godot-JSON. Nothing here yet but added the framework
## in case I decide to expand upon this more.
static var _default_configs: Array[SerializationObjectConfig]

## User (your) configurations to be registered to the global [SerializationImpl]. 
## ALL configs need to be registered in order to deserialize objects. It is important
## to not remove [SerializationObjectConfig]s without sanitizing stored JSON data that was serialized
## by those config(s), as the [member SerializationObjectConfig.id] is stored in the JSON data
## to determine how to deserialize it. So removing its conifg will cause errors.
@export var _user_configs: Array[SerializationObjectConfig]:
	set(value):
		_user_configs = value
...
var _configs: Array[SerializationObjectConfig]

## [member SerializationObjectConfig.id]:[SerializationObjectConfig]
var _configs_by_id: Dictionary

## [member SerializationObjectConfig.for_class]:[SerializationObjectConfig]
...
func has_config(config: SerializationObjectConfig) -> bool:
	return _configs.has(config)


## Returns true if a [SerializationObjectConfig] with [member SerializationObjectConfig.for_class] as [param _class]
...
func editor_has_class(_class: StringName) -> bool:
	for config: SerializationObjectConfig in _default_configs:
		if config.for_class == _class:
			return true
	for config: SerializationObjectConfig in _user_configs:
		if config.for_class == _class:
			return true
	return false


## Returns true if a [SerializationObjectConfig] with [member SerializationObjectConfig.id] as [param id] exists,
...
## Returns the [SerializationObjectConfig] with [member SerializationObjectConfig.for_class] of [param _class], 
## or null if one does not exist. For use at runtime only.
func get_config_by_class(_class: StringName) -> SerializationObjectConfig:
	assert(!Engine.is_editor_hint(), "get_config_by_class not for use in the Editor")
	return _configs_by_class.get(_class, null)


## Returns the [SerializationObjectConfig] with [member SerializationObjectConfig.id] of [param id], 
## or null if one does not exist. For use at runtime only.
func get_config_by_id(id: StringName) -> SerializationObjectConfig:
...
## starts up).
func add_config(config: SerializationObjectConfig) -> void:
...
## saved to this resource (will need to be manually called each time the engine/game
## starts up).
func remove_config(config: SerializationObjectConfig) -> void:
...
## [SerializationImpl]s. Duplicates internal arrays/dictionaries but
## keeps the objects in them the same instances.
func copy() -> SerializationObjectConfigRegistry:
	assert(!Engine.is_editor_hint(), "copy() not supported in the Editor")
	var registry: SerializationObjectConfigRegistry = SerializationObjectConfigRegistry.new()
...
func _populate_dictionaries(array: Array[SerializationObjectConfig], array_name: String) -> void:
	assert(!Engine.is_editor_hint(), "_populate_dictionaries not supported in the Editor")
	# Add to dictionaries
	for config: SerializationObjectConfig in array:
		if config == null:
